...
def h264_encoder(filename, output, resolutionH, bitrate):
    ...
    cmd=["ffmpeg", "-y", "-i", filename, "-filter:v", f"scale=-2:{resolutionH}", "-c:v", "libx264", "-b:v", f"{bitrate}",\
        "-r", "24", "-x264opts", "keyint=48:min-keyint=48:no-scenecut", \
        "-movflags", "faststart", "-bufsize", "8600k", \
        "-pix_fmt", "yuv420p", "-profile:v", "main", "-preset", "veryfast", "-an", output]
    run_ffmpeg_process(cmd)
...
def aac_encoder(filename, output):
    ...
    # ffmpeg -y -i {filename} -map 0:1? -filter:a loudnorm -vn -c:a aac -b:a 128k -ar 48000 -ac 2 {outputfile}"
    cmd = ["ffmpeg", "-y", "-i", filename, "-map", "0:1?", "-filter:a", "loudnorm", \
        "-vn", "-c:a", "aac", "-b:a", "128k", "-ar", "48000", "-ac", "2", output]
    run_ffmpeg_process(cmd)
